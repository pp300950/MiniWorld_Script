...
local _, current_epoch = VarLib2:getGlobalVarByName(3, "current_epoch")
...
for pair in input_iterator do  --ช้iteratorแทนอาเรย์
    local x1, x2 = pair:match("%s*(%-?%d+%.?%d*)%s*,%s*(%-?%d+%.?%d*)%s*")
    ...
    if x1 and x2 and target then
        ...
        if prediction ~= prediction then  --ตรวจNaN
        ...
        -- แสดงผลโดยไม่ย่อ
VarLib2:setGlobalVarByName(4, "w1",string.format("%.20f", w1))
VarLib2:setGlobalVarByName(4, "w2",string.format("%.20f", w2))
VarLib2:setGlobalVarByName(4, "b", string.format("%.20f", b))
    else
        print("รูปแบบข้อมูลผิดพลาด:", pair)
    end
end
